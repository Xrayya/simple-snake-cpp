#include "app_core/input/tui_input.hpp"
#include <memory>
#include <termios.h>

TUIInputHandler::TUIInputHandler(int timeout_ms) : timeout_ms(timeout_ms) {}

auto TUIInputHandler::poll() const -> std::unique_ptr<event::Input> {
  struct termios oldt{};
  struct termios newt{};
  tcgetattr(STDIN_FILENO, &oldt); // save old settings
  newt = oldt;
  newt.c_lflag &= static_cast<unsigned int>(
  ...
  int ready_fds = select(STDIN_FILENO + 1, &set, nullptr, nullptr, &timeout);
  ...
  if (chr != -1) {
    ...
    case 27: {
      ...
      if (getchar() == 91) {
        switch (getchar()) {
        case 'A':
          return std::make_unique<event::KeyInput>(KeyCode::ArrowUp, modifiers,
                                                   false);
        case 'B':
          return std::make_unique<event::KeyInput>(KeyCode::ArrowDown,
                                                   modifiers, false);
        case 'C':
          return std::make_unique<event::KeyInput>(KeyCode::ArrowRight,
                                                   modifiers, false);
        case 'D':
          return std::make_unique<event::KeyInput>(KeyCode::ArrowLeft,
                                                   modifiers, false);
        default:
          break;
        }
      } else {
        return std::make_unique<event::KeyInput>(KeyCode::Escape, modifiers,
                                                 false);
      }
      break;
    }
    ...
    return std::make_unique<event::KeyInput>(KeyCode::Unknown, KeyModifiers{},
                                             false);
  }
